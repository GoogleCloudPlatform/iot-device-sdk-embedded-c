...
#define IOTC_CM_PRINT_ERROR_COLORED( ansi_color, text ) \
    print_error( ansi_color "%s" ANSI_COLOR_RESET, text );

#define IOTC_CM_PRINT_MESSAGE_COLORED( ansi_color, text ) \
                                           ...
                                           size_t milliseconds_elapsed)
{
    ...
    IOTC_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_GREEN, "[==========]");
    print_message(" %u test(s) run. (%lu ms)\n", (unsigned)total_executed, milliseconds_elapsed);
    IOTC_CM_PRINT_ERROR_COLORED( ANSI_COLOR_GREEN, "[  PASSED  ]");
    print_error(" %u test(s).\n",
                (unsigned)(total_passed));

    if (total_failed) {
        IOTC_CM_PRINT_ERROR_COLORED( ANSI_COLOR_RED, "[  FAILED  ]");
                                          ...
                                          size_t milliseconds_elapsed)
{
    ...
    case PRINTF_TEST_START:
        IOTC_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_GREEN, "[ RUN      ]");
        print_message(" %s\n", test_name);
        break;
    case PRINTF_TEST_SUCCESS:
        IOTC_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_GREEN, "[       OK ]");
    ...
    case PRINTF_TEST_FAILURE:
        ...
        IOTC_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_RED, "[  FAILED  ]");
        print_message(" %s\n", test_name);
        break;
    case PRINTF_TEST_SKIPPED:
        IOTC_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_YELLOW, "[  SKIPPED ]");
        print_message(" %s\n", test_name);
        break;
    case PRINTF_TEST_ERROR:
        ...
        IOTC_CM_PRINT_ERROR_COLORED( ANSI_COLOR_RED, "[  ERROR   ]");
        print_error(" %s\n", test_name);
        break;
    }
}
                            ...
                            CMFixtureFunction group_teardown)
{
    ...
    t0_all = iotc_bsp_time_getcurrenttime_milliseconds();
    ...
    if (rc == 0) {
        ...
        for (i = 0; i < num_tests; i++) {
            ...
            t0_testcase = iotc_bsp_time_getcurrenttime_milliseconds();

            rc = cmocka_run_one_tests(cmtest);


            deltatime_testcase = iotc_bsp_time_getcurrenttime_milliseconds() - t0_testcase;
    ...
    deltatime_all = iotc_bsp_time_getcurrenttime_milliseconds() - t0_all;
